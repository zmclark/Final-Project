...
*/
	...
	if (lines[0] != "Hello, Trainer! Welcome to the world of EECS 183 Creature Trainer!") {
		opponentDamage = lines[5];
		inSS.clear();
		inSS.str(opponentDamage);
		...
		inSS >> elementType;
		inSS >> fore;
		inSS >> enemyDamage;    //reads enemy damage into int variable for later comparison
		getElementType(enemyName);
	}
	...
	string s3 = "s3";
	string s4 = "s4";
	istringstream attackInput(a);
	istringstream swapOneInput(s1);
	istringstream swapTwoInput(s2);
	istringstream swapThreeInput(s3);
	istringstream swapFourInput(s4);
	

	// Finds the creature with the most HP
	int highestHP = 0;
	for (int p = 0; p < 4; p++){
		if (creatureHealth[p] > creatureHealth[highestHP]){
			highestHP = p;
		}
	}

	//Swaps out creature if it is about to die to a creature with the highest HP
	if (creatureHealth[currentCreature] <= enemyDamage){
		if (highestHP == 0){
			swapOneInput >> response; 
		}
		else if (highestHP == 1){
			swapTwoInput >> response;
		}
		else if (highestHP == 2){
			swapThreeInput >> response;
		}
		else if (highestHP == 3){
			swapFourInput >> response;
		}
		currentCreature = highestHP;
	}
	else{
		attackInput >> response;
	}
...
void Trainer::getElementType(string enemyName){
	for (int i = 0; i < 26; ++i) {
		char firstLetter = static_cast<char>(i + 65);
		if (enemyName.at(0) == firstLetter) {
			creatureElement[i] = elementType;
		}
	}
}
